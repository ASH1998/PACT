#!/usr/bin/env python3
"""PACT CLI for the Claude Code plugin.

Gives Claude Code a slash-command style interface:
...
It uses the existing PACT v1 REST API. The companion ``pact_hook.py`` reuses the
helpers here so that native Claude Code tool calls (Bash, Read, Edit, WebFetch)
are routed through the same PACT decision path and show up live in the
dashboard.
"""
...
# plugins/pact-claude -> plugins -> repo root
...
STATE_PATH = REPO_ROOT / ".pact" / "claude-session.json"
...
def sign_envelope(envelope: dict[str, Any], private_key_b64: str) -> None:
    try:
        ...
        vpy = venv_python()
        # Don't re-exec inside the PreToolUse hook here: by this point the hook
        # has already consumed its stdin event, so a fresh process would read
        # nothing and silently defer. Interpreter selection for the hook happens
        # earlier -- in the launcher (run_hook / run_hook.cmd) and in
        # pact_hook.bootstrap_venv(), which both forward stdin correctly.
        in_hook = "pact_hook" in os.path.basename(sys.argv[0] or "") if sys.argv else False
        if vpy and not in_hook and os.environ.get("PACT_CLAUDE_REEXECED") != "1":
            import subprocess

            env = os.environ.copy()
            env["PACT_CLAUDE_REEXECED"] = "1"
            # Re-run the same command under the venv interpreter, which has
            # PyNaCl. subprocess.run behaves identically on POSIX and Windows
            # (os.execve does not: on Windows it detaches and loses stdout),
            # and we propagate the child's exit code.
        ...
        raise PactError(
            ...
            "plugins/pact-claude/scripts/pact_cli.py ..."
        ) from exc
...
def load_state(required: bool = True) -> dict[str, Any] | None:
    if not STATE_PATH.exists():
        if required:
            raise PactError("No PACT Claude session exists. Run /pact start first.")
        return None
...
def run_check(
    ...
    """Mint a capability, sign an envelope, and run it through the gateway.

    Mutates and persists ``state`` (step_id, parent_action_hash, last_decision)
    and returns the backend decision dict. Shared by the CLI ``check`` command
    ...
    decision = request_json(
        "POST",
        "/v1/gateway/execute",
        {"run_id": state["run_id"], "envelope": envelope, "skip_approval": approved},
    )
    ...
    save_state(state)
    decision.setdefault("tool", tool)
    decision.setdefault("resource", resource)
    return decision
...
def start_cmd(args: argparse.Namespace) -> None:
    ...
    agent_id = args.agent_id or f"claude-{uuid.uuid4().hex[:8]}"
    ...
    run = request_json(
        "POST",
        "/v1/runs",
        {"agent_id": agent_id, "scenario_name": "claude_plugin", "user_goal": args.goal},
    )
    state = {
    ...
    json_out(
        {
            ...
            "Native Claude Code tool calls are now PACT-checked via the PreToolUse hook.",
        }
    )
...
def check_cmd(args: argparse.Namespace) -> None:
    ...
    decision = run_check(state, args.tool, tool_args, approved=args.approved)
    json_out(
        {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PACT Claude Code plugin CLI")
    sub = parser.add_subparsers(dest="command", required=True)

    sub.add_parser("health").set_defaults(func=health_cmd)

    start = sub.add_parser("start")
    start.add_argument("--goal", default="PACT-protected Claude Code session")
